//! CORE (pure). Gesture feel primitives — velocity estimation over a
//! finger's recent samples, momentum projection, and the rubber-band
//! boundary curve.
//!
//! Three pure computations:
//!
//!   1. VELOCITY — a finite difference over a short ring of timestamped
//!      samples, so release velocity is the finger's real speed, not a
//!      single-frame delta.
//!   2. PROJECTION — where the content WOULD come to rest if it decelerated
//!      naturally from that velocity. Settle decisions (open the drawer?
//!      close it?) test the projected end, never the raw release position —
//!      a small flick still sends a panel all the way, because the interface
//!      predicted the intent.
//!   3. RUBBER-BAND — resistance past a hard limit that grows nonlinearly
//!      and springs back on release. The absence of hard walls is most of
//!      what "polished" feels like.
//!
//! The settle spring itself is NOT here: `spring.zig` already owns damped
//! harmonic motion. Callers seed `spring.stepScalar` with the release
//! velocity from here — that hand-off is the whole integration point.
//!
//! PURE: no clock, no RNG, no I/O, no allocation. Timestamps are
//! caller-supplied milliseconds; only differences are ever taken.

const std = @import("std");
const assert = std.debug.assert;

// ---------------------------------------------------------------------------
// Feel parameters. Named constants, tuned in one place, never inline.
// ---------------------------------------------------------------------------

/// Per-millisecond velocity retention of a naturally decelerating fling.
/// Feeds the projection's closed form; also the friction a caller applies
/// if it decays a fling per-ms.
pub const decel_rate: f32 = 0.998;

/// The projection horizon in ms-equivalents: sum of the geometric decay,
/// r/(1-r). At r = 0.998 this is 499 — a fling coasts about half a second
/// of its release velocity into distance.
...
/// Rubber-band firmness: 0.55 is the classic scroll-view coefficient.
/// Lower is stiffer (less give past the edge).
pub const rubber_coeff: f32 = 0.55;

/// Below this speed a release is a placement, not a flick: the settle
/// decision falls back to position alone. In units-per-second of whatever
/// axis the caller measures.
...
/// PLAIN DATA. One timestamped pointer sample — the value that crosses
/// the shell→core boundary. `t_ms` is a monotonic millisecond count; only
...
pub const PointerSample = struct {
    ...
    comptime {
        // Budget: 4 + 4 + 4 = 12, no padding.
        assert(@sizeOf(PointerSample) == 12);
    }
};

/// Ring capacity. At 120 Hz input, ~100 ms of history is well under 32
/// samples.
pub const ring_cap = 32;

/// A fixed ring of recent samples for ONE pointer. Struct-of-arrays
/// inside: the velocity sweep reads the t column first and touches x/y
/// only at the two ends it selects. Inline fixed arrays — never grows,
/// never allocates.
pub const SampleRing = struct {
    ...
    comptime {
        // Budget: 3 columns * 32 * 4 = 384, + 2 u8, padded to 388.
        assert(@sizeOf(SampleRing) == 388);
    }
};

/// Push one sample, overwriting the oldest once full.
...
/// A 2-D velocity in units/s.
pub const Velocity = struct {
    x: f32,
    y: f32,
};

/// Estimate the finger's velocity from the ring: the finite difference
/// between the newest sample and the oldest sample still inside
/// `velocity_window_ms`. Robust to batched delivery and to a press-and-hold
/// before release (old motion ages out and the estimate decays to zero).
...
/// Where a value coasting at `vel_per_s` from `pos` would come to rest
/// under natural deceleration — the closed form of the geometric decay, no
...
/// The settle decision for a 0..1 surface fraction (drawer, sheet): open
/// when the PROJECTED end passes halfway. A release slower than
/// `flick_min_per_s` carries no directional intent, so it degrades to
/// position alone.
...
/// asymptotic curve, sign-preserving. Zero at zero, monotone, and bounded
/// by `dim` (the view dimension) — the finger can pull forever and the
/// content approaches but never passes one screen of give.
...
/// — the shell hands the springing offset back to the drag as raw travel,
/// and the finger resumes the same stretch with no snap.
...
/// The band's local slope at raw travel `offset` — the factor that converts
/// a finger velocity into displayed-give velocity at the moment of release,
/// so the spring-back starts at the speed the stretch was visibly moving.
...
test "rubber-band: zero at zero, monotone, bounded, sign-symmetric" {
    ...
    while (x < 10_000) : (x += 100) {
        ...
        try testing.expect(b < dim);
        try testing.expectEqual(-b, rubberBand(-x, dim));
        prev = b;
    }
