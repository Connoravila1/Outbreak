...
| **O3** 🟡 | **Cell size (39 bits)** — was **the most consequential unvalidated number in the project** | **Measured, and the steady-state flicker fear did not materialise.** Pixel 10 Pro, indoors (the hard case — the game is played indoors), diagnostic build fixing GPS once a second, phone held still in one spot. Over ~230 fixes: **three room changes, all during the first couple of minutes as the receiver converged** from a coarse estimate to a tight lock — then **40+ consecutive fixes with zero change, at 1 m reported accuracy**, one metre inside a 24 m cell. The concern was #2 below: *"a player sitting still would flicker between cells and break their own quorum."* **In steady state, on this hardware, a stationary phone does not flicker at all.** **The transient convergence changes are real and expected, and the architecture already handles them:** `gps.zig`'s `patience` field requires N consecutive same-room fixes before a room is trusted, which is exactly the convergence-absorbing mechanism — those early changes would never reach the server. What is NOT yet proven: cheaper phones with worse receivers (the 1 m lock here suggests a good sky view — a basement would differ), dense-urban canyons, and the *feel* half of O3 (*"six people in this coffee shop"*), which needs multiplayer that does not exist yet. So: **de-risked, not universally closed.** Re-measure on a low-end device before it decides anything for launch. *(Original forces for coarser, kept for the record: (1) GPS barely works indoors; (2) the flicker fear, now seen only as transient convergence; (3) privacy. Against them, the fantasy wants the cell to BE the room. The battery argument was withdrawn — with geofencing, GNSS costs 0.02%/8 h.)* Validated with the on-phone readout: `./android/package.sh --diagnostic`. **Update 2026-07-16:** the shell governor now also treats a room change that *bounces straight back* as jitter, not a move, so a still phone settles even when the receiver briefly flickers — and an 8-hour overnight run field-confirmed the payoff: GPS asleep ~99% of the night (10 fixes in 8.7 h). | **Phase 3 — measured 2026-07-14** |
