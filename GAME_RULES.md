...
| **O3** 🔴 | **Cell size (39 bits)** — **the most consequential unvalidated number in the project** | **Three forces say *coarser*: (1) GPS barely works indoors and this game is played indoors — wifi positioning is *better* there, not just cheaper; (2) a 24 m cell is inside urban GPS error, so a player sitting still would flicker between cells and break their own quorum; (3) privacy.** Against them, one force says finer: the fantasy — *"six people in this coffee shop"* only lands if the cell **is** the coffee shop. *(The battery argument was **withdrawn**: with geofencing, GNSS costs 0.02%/8 h. Coarsening to save power would be optimizing a rounding error — G3 says stop.)* **Answerable in Phase 3, by measuring positioning error in a real café.** | **Phase 3** |
